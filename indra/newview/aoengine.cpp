...
void AOEngine::tick()
{
	const LLUUID categoryID=gInventory.findCategoryByName(ROOT_FOLDER);

	if(categoryID.isNull())
	{
		llwarns << "no " << ROOT_FOLDER << " folder yet. Creating ..." << llendl;
		gInventory.createNewCategory(gInventory.getRootFolderID(),LLFolderType::FT_NONE,ROOT_FOLDER);
		mAOFolder.setNull();
	}
	else
	{
